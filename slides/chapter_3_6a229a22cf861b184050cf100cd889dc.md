...
DESC EMPLOYEE
```{{1}}
![](https://assets.datacamp.com/production/repositories/4213/datasets/b139c637a9b175482b15c6b3428947332f0beef7/descTable.png){{2}}
...
The Query:
```sql
WITH hierarchy (Id, Name, JobTitle,
Level,Supervisor) AS 
( SELECT
      Id, Name, JobTitle, 0, Supervisor
  FROM
  ...
  UNION ALL 
  SELECT
     emp.Id, emp.Name, emp.JobTitle,
Level + 1,emp.Supervisor
  FROM
     dbo.Employee emp
  INNER JOIN OrganisationChart ON 
     emp.Supervisor = OrganisationChart.Id
...
`@script`
Now let's have a look at the result of the query. Each employee has its supervisor and its hierarchical level.
...
`@script`
3 has 5 as boss, 7 has 2 and so. The level is highlighted with yellow.
This knowledge of the organization can be very usefull and it is quite easy to be done by hierarchical queries. Let's practice more with hierarchical queries. we want to query all employess managed my an specific employee.
...
`@script`
To do so we have to modify the query as described here. We add the value root which corresponds to the top-most supervisor of each employee. Doing so, we can now query all employees managed by the employee with ROOT ID 5
...
The Query:
```sql
WITH hierarchy (Id,Name,JobTitle,Level,
Supervisor, Root) AS
(   SELECT
...
`@script`
This results looks like this. You can see that the employee 4 has 2 employees to manage.
...
`@script`
The new Root value is highlighted in yellow.
